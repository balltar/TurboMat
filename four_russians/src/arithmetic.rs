...
pub fn dot_gf2(a: &[Word], b: &[Word]) -> u32 {
    assert_eq!(a.len(), b.len());
...
/// ```
#[inline]
pub fn trunc_dot_gf2(a: &[Word], b: &[Word], i0: Index, i1: Index) -> u32 {
    assert_eq!(a.len(), b.len());
    let mut product: Word = 0;
    for i in i0..i1 {
        product ^= a[i] & b[i]
...
/// ```
#[inline]
pub fn add_rows(a: &mut [Word], b: &[Word], c: &[Word], i0: Index, i1: Index) {
    for i in i0..i1 {
        a[i] = b[i] ^ c[i]
    }
}
